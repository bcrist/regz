...
fn writeDescription(
    ...
    // TODO maybe remove SVD newlines if there are only one or two words on a second line?
    // Sometimes SVDs have superfluous wrapping.
    ...
    if (db.interrupts.items.len > 0 and db.cpu != null) {
        if (svd.CpuName.parse(db.cpu.?.name.?)) |cpu_type| {
            ...
            if (cpu_type != .avr) {
                ...
                while (address < 0x2C) : (address += 4) {
                    try writer.print(
                        \\    _reserved{X}: u32 = undefined,
                        \\
                    , .{ address });
                }
            ...
            for (db.interrupts.items) |interrupt| {
                ...
                while (expected < interrupt.value) : (expected += 1) {
                    try writer.print("    _reserved{X}: u32 = undefined,\n", .{ address });
                    address += 4;
                }
        ...
        .namespaced => if (db.registers_in_clusters.get(cluster_idx)) |range| {
            ...
            if (dimension_opt) |dimension| {
                ...
                while (i < num_padding_words) : (i += 1) {
                    try writer.print("_reserved{}: u{} = undefined,\n", .{ i, db.device.?.width });
                }
...
pub fn getRegister(
    ...
    return Register{
        ...
        .type_override = register.type_override,
        .size = size,
        .access = access,
